...
/**
 * <p>システム名： CUSEXT</p>
 * <p>業　務　名： 不成約情報操作クラス</p>
 * <p>概　　　要： 不成約情報の取得､登録､修正を行う。</p>
 * <p>作　成　日： 2010/10/22</p>
 *
 * @version 1.0.0
...
public class DisAgrInfoDAO extends DAOBase {
    ...
    public synchronized DisAgrmntInfoVO insertDisAgrInfo(DisAgrEntryForm disAgrEntryForm, String opeId) throws Exception, SQLException {
        ...
        try {
            ...
            /** 2016/05/18 王　旭 ADA IAライン対応Phase2　1.1 CUSEXTでのデータ登録先変更 ADD Begin */
            ...
            /** 2016/05/18 王　旭 ADA IAライン対応Phase2　1.1 CUSEXTでのデータ登録先変更 ADD End */
    ...
    public synchronized DisAgrmntInfoVO updateDisAgrInfo(DisAgrEntryForm disAgrEntryForm, String opeId) throws Exception, SQLException {
        ...
        try {
            ...
            strSql.append("  ,UPDATE_DT = SYSDATE");                                        // 更新日時
            strSql.append("  ,UPDATE_BY = ? ");                                             // 更新者ID
            strSql.append(" where ");
            strSql.append("  AGRMNT_NO = ?");         // 不成約No
    ...
    private String getDisAgrInfoSql(boolean isDetail, boolean isBranch) {
        ...
        if(isDetail) {
            sqlSb.append("  ,TO_CHAR(AGR.ENTRY_DT, 'YYYY/MM/DD HH24:MI') AS ENTRY_DT ");        // 登録日時
            sqlSb.append("  ,TO_CHAR(AGR.UPDATE_DT, 'YYYY/MM/DD HH24:MI') AS UPDATE_DT ");      // 更新日時
            ...
            sqlSb.append("  ,TO_CHAR(AGR.ENTRY_DT, 'YYYY/MM/DD') AS ENTRY_DT ");                // 登録日時
            sqlSb.append("  ,TO_CHAR(AGR.UPDATE_DT, 'YYYY/MM/DD') AS UPDATE_DT ");              // 更新日時
        }
        sqlSb.append("from ");
